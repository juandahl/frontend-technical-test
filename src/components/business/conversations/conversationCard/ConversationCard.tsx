...
const ConversationCard: React.FC<ConversationCardProps> = ({ conversation, userId }) => {
  // Variables
  const userName = conversation.senderId === userId ? conversation.recipientNickname : conversation.senderNickname;
  const lastMessageDate = new Date(Date.now() - conversation.lastMessageTimestamp);

  return (
    <Container href={`/${conversation.id}`}>
      <CustomAvatar>
        {getInitialsFromFullName(conversation.senderNickname)}
      </CustomAvatar>
